...
export async function weatherRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/forecast/hourly", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    url.searchParams.set("latitude", String(latitude));
    url.searchParams.set("longitude", String(longitude));
    url.searchParams.set("hourly", "temperature_2m,weathercode,precipitation_probability,windspeed_10m,is_day,uv_index");
    url.searchParams.set("timezone", "auto");
    url.searchParams.set("start_date", date);
    url.searchParams.set("end_date", date);

    let rawData: { hourly?: { time?: string[]; temperature_2m?: number[]; weathercode?: number[]; precipitation_probability?: number[]; windspeed_10m?: number[]; is_day?: number[]; uv_index?: number[] } };
    try {
      const res = await fetch(url.toString());
      if (!res.ok) {
        throw new Error(`Open-Meteo returned ${res.status}`);
